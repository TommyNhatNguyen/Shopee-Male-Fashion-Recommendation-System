...
# ---------
    ...
    if have_account == log_in_check[0]:
        ...
        if user_search != "":
            st.write("## Search results for `{}` 👇".format(user_search))
    ...
    # Trường hợp 2: Collaborative Filtering ----------------
        ...
        user_id_select = als_recommendation[als_recommendation['user'] == login_account]['user_id'].values[0]
        num_product_show = st.select_slider('Select number of recommend products:', [5, 10, 15, 20, 30, 50, 100], value=5, key='num_product_show1')
        if login_account in list_of_users:
            st.markdown('## You chose user name `{}` with user_id: `{}` '.format(login_account, user_id_select))
